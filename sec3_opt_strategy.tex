...
IRLS là viết tắt của \textit{Iteratively Reweighted Least-Squares}, tức bình phương tối thiểu có trọng số lặp. Ý tưởng của phương pháp là biến một bài toán không hoàn toàn thuộc dạng least-squares thành một chuỗi các bài toán least-squares có trọng số.
\medskip
...
\begin{equation}
    E_{\mathrm{IRLS}}(x)
    =
    \sum_{i=1}^{m} f_i(x)\,g_i(x)^2.
    \label{eq_irls_general}
\end{equation}
Trong biểu thức này, $f_i(x)$ đóng vai trò là trọng số của sai số bình phương $g_i(x)^2$. Nếu tại bước lặp thứ $k$ trọng số được cố định ở
\begin{equation}
    w_i^{(k)}=f_i(x_k),
...
Nếu $g_i(x)$ là hàm tuyến tính, bài toán \eqref{eq_irls_iteration} là weighted least-squares tuyến tính. Nếu đặt
\begin{equation}
    g_i(x)=a_i^{T}x-b_i,
...
Cho $A\in\mathbb{R}^{m\times n}$ và $b\in\mathbb{R}^{m}$. Bài toán least-squares tuyến tính cổ điển tối thiểu hóa $\|Ax-b\|_2^2$. Một tổng quát hóa tự nhiên là thay chuẩn $\ell_2$ bằng chuẩn $L^p$:
\begin{equation}
    E_p(x)=\|Ax-b\|_p^p
    =
...
\begin{itemize}
    \item Nếu residual hoặc hệ số gần bằng $0$, trọng số có thể rất lớn, làm hệ tuyến tính bị kém điều kiện.
...
Với một số bài toán sparse recovery, hội tụ của IRLS đã được chứng minh khi thuật toán được điều chỉnh phù hợp \cite{Daubechies2010IRLS}. Khi triển khai, vẫn cần theo dõi điều kiện dừng, chất lượng nghiệm và độ ổn định của hệ tuyến tính ở mỗi vòng lặp.
