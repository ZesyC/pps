\section{Coordinate Descent và Alternation}
...
\end{equation}
Chiến lược này được gọi là \textbf{alternating optimization}.
\medskip
...
\subsection{Khi nào nên dùng alternation}
...
\begin{itemize}
    \item Bài toán gốc phi tuyến theo toàn bộ biến, nhưng tuyến tính hoặc least-squares theo từng khối biến.
...
\subsubsection*{PCA tổng quát và Alternating Least-Squares}
...
Nếu cố định $C$, bài toán tối ưu $Y$ là least-squares. Nếu cố định $Y$, bài toán tối ưu $C$ cũng là least-squares. Vì vậy, ta có thuật toán alternating least-squares:
\begin{equation}
    \begin{cases}
        Y_{k+1}=\arg\min_Y \|X-C_kY\|_{\mathrm{Fro}}^2,\\
        C_{k+1}=\arg\min_C \|X-CY_{k+1}\|_{\mathrm{Fro}}^2.
    \end{cases}
...
với ràng buộc
\begin{equation}
    R_v^{T}R_v=I_2,\qquad \forall v\in V.
    \label{eq_arap_rotation_constraint}
\end{equation}
Ở đây $x_v$ là vị trí ban đầu, $y_v$ là vị trí sau biến dạng, còn $R_v$ là ma trận quay cục bộ tại đỉnh $v$.
\medskip

Nếu tối ưu đồng thời tất cả $R_v$ và $y_v$, bài toán rất khó vì có ràng buộc trực giao. Nhưng nếu tách biến, ta có hai bước:
\begin{itemize}
    \item Cố định các $R_v$, tối ưu các $y_v$. Khi đó bài toán là least-squares thưa, có thể giải bằng hệ tuyến tính xác định dương.
...
Ví dụ này cho thấy sức mạnh của alternation: một bài toán phi tuyến lớn được thay bằng một bước giải hệ tuyến tính thưa và nhiều bài toán nhỏ có nghiệm đóng.

\subsubsection*{Coordinate descent cho least-squares}
...
\subsubsection*{Thuật toán k-means clustering}
...
\end{equation}
K-means chính là alternation giữa hai bước \cite{Lloyd1982}:
\begin{itemize}
...
\subsection{Augmented Lagrangian và ADMM}
\label{SUB_SEC_admm}
...
\begin{cy}
    Một số tài liệu viết Lagrangian dưới dạng $f(x)-\lambda^{T}g(x)$ và cập nhật dual với dấu trừ. Hai cách viết này tương đương nếu thay $\lambda$ bởi $-\lambda$. Trong báo cáo này, ta dùng quy ước dấu cộng để nhất quán với công thức ADMM ở các ví dụ phía sau.
\end{cy}

\subsubsection*{Phương pháp Augmented Lagrangian}
Augmented Lagrangian kết hợp hai ý tưởng trên:
\begin{equation}
    \mathcal{L}_{\rho}(x,\lambda)
    =
    f(x)+\lambda^{T}g(x)+\frac{\rho}{2}\|g(x)\|_2^2.
...
\end{equation}
Thành phần phạt bậc hai giúp bước tối ưu theo $x$ không đi quá xa khỏi tập ràng buộc, còn nhân tử Lagrange giúp không cần đưa $\rho$ tiến tới vô hạn.

\subsubsection*{ADMM: công thức tổng quát}
ADMM, tức \textit{Alternating Direction Method of Multipliers}, áp dụng ý tưởng augmented Lagrangian cho bài toán có hai nhóm biến:
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    ...
    \label{eq_admm_steps}
\end{equation}
Khác với augmented Lagrangian cổ điển, ADMM không tối ưu đồng thời $x$ và $z$. Thay vào đó, thuật toán tối ưu luân phiên từng biến. Việc tách này có thể làm tăng số vòng lặp, nhưng mỗi vòng lặp thường rẻ hơn nhiều và dễ song song hóa hơn \cite{Boyd2011ADMM}.

\subsubsection*{Nhận xét về hội tụ và tham số $\rho$}
Trong trường hợp $f$ và $h$ là các hàm lồi, ràng buộc là tuyến tính và augmented Lagrangian có điểm yên ngựa, ADMM có bảo đảm hội tụ tới nghiệm tối ưu toàn cục \cite{Boyd2011ADMM}. Với bài toán không lồi, ADMM vẫn thường được dùng trong thực nghiệm, nhưng lý thuyết hội tụ yếu hơn và kết quả có thể phụ thuộc vào cách tách biến cũng như khởi tạo.
\medskip
...
Tham số $\rho>0$ điều khiển mức phạt đối với vi phạm ràng buộc. Nếu $\rho$ quá nhỏ, biến primal có thể tiến chậm tới miền thỏa ràng buộc. Nếu $\rho$ quá lớn, bước cập nhật có thể trở nên kém điều kiện hoặc quá tập trung vào ràng buộc mà giảm ít hàm mục tiêu. Trong triển khai, người ta thường theo dõi primal residual và dual residual để điều chỉnh $\rho$: nếu primal residual lớn hơn nhiều thì tăng $\rho$, còn nếu dual residual lớn hơn nhiều thì giảm $\rho$.

\subsubsection*{Ví dụ: Nonnegative least-squares bằng ADMM}
Xét bài toán
\begin{equation}
    \begin{aligned}
...
\subsubsection*{Ví dụ: Geometric median bằng ADMM}
...
\begin{equation}
    \begin{aligned}
        ...
        &\text{subject to}\quad z_i+x=x_i,\qquad i=1,\dots,N.
    \end{aligned}
