...
\begin{cy}
    Hai quy ước dấu $f(x)-\lambda^{T}g(x)$ và $f(x)+\lambda^{T}g(x)$ là tương đương nếu thay $\lambda$ bởi $-\lambda$. Để khớp với phần Augmented Lagrangian cổ điển trong tài liệu, đoạn này dùng dấu trừ. Riêng công thức ADMM phía sau được viết theo đúng quy ước dấu cộng xuất hiện trong tài liệu.
\end{cy}
...
\begin{equation}
    \begin{cases}
        \lambda_{k+1}=\lambda_k-\rho g(x_k),\\
        x_{k+1}=\arg\min_x \mathcal{L}_{\rho}(x,\lambda_{k+1}).
    \end{cases}
